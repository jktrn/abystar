import { TalentScaling, Bonus, Character } from '@/interfaces/Character'

const talentScalings: TalentScaling = {
    // ...
}

const characterBonuses: Bonus[] = [
    // ...
]

const constellationBonuses: Bonus[] = [
    // ...
]

const Ningguang: Character = {
    name: 'Ningguang',
    icon: '/images/characters/UI_AvatarIcon_Ningguang.png',
    weapon: 'Catalyst',
    vision: 'Geo',
    rarity: 4,
    description:
        'The Tianquan of the Liyue Qixing. Her wealth is unsurpassed in all of Teyvat.',
    occupation: 'Liyue Qixing',
    baseStats: {
        '1/20': {
            'Base HP': 821.0,
            'Base ATK': 17.81,
            'Base DEF': 48.07,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.0,
        },
        '20/20': {
            'Base HP': 2108.0,
            'Base ATK': 45.75,
            'Base DEF': 123.49,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.0,
        },
        '20/40': {
            'Base HP': 2721.0,
            'Base ATK': 59.05,
            'Base DEF': 159.4,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.0,
        },
        '40/40': {
            'Base HP': 4076.0,
            'Base ATK': 88.45,
            'Base DEF': 238.76,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.0,
        },
        '40/50': {
            'Base HP': 4512.0,
            'Base ATK': 97.91,
            'Base DEF': 264.28,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.06,
        },
        '50/50': {
            'Base HP': 5189.0,
            'Base ATK': 112.62,
            'Base DEF': 303.98,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.06,
        },
        '50/60': {
            'Base HP': 5770.0,
            'Base ATK': 125.22,
            'Base DEF': 338.0,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.12,
        },
        '60/60': {
            'Base HP': 6448.0,
            'Base ATK': 139.93,
            'Base DEF': 377.71,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.12,
        },
        '60/70': {
            'Base HP': 6884.0,
            'Base ATK': 149.38,
            'Base DEF': 403.22,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.12,
        },
        '70/70': {
            'Base HP': 7561.0,
            'Base ATK': 164.07,
            'Base DEF': 442.88,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.12,
        },
        '70/80': {
            'Base HP': 7996.0,
            'Base ATK': 173.53,
            'Base DEF': 468.39,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.18,
        },
        '80/80': {
            'Base HP': 8674.0,
            'Base ATK': 188.24,
            'Base DEF': 508.1,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.18,
        },
        '80/90': {
            'Base HP': 9110.0,
            'Base ATK': 197.69,
            'Base DEF': 533.61,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.24,
        },
        '90/90': {
            'Base HP': 9787.0,
            'Base ATK': 212.4,
            'Base DEF': 573.32,
            'CRIT Rate': 0.05,
            'CRIT DMG': 0.5,
            'Geo DMG Bonus': 0.24,
        },
    },
    talents: [
        {
            name: 'Normal Attack: Sparkling Scatter',
            image: '/images/characters/Skill_A_Catalyst_MD.png',
            description:
                'Normal Attack\nShoots gems that deal\nGeo DMG\n.\nUpon hit, this grants Ningguang 1 Star Jade.\nCharged Attack\nConsumes a certain amount of stamina to fire off a giant gem that deals\nGeo DMG\n.\nIf Ningguang has any Star Jades, unleashing a Charged Attack will cause the Star Jades to be fired at the enemy as well, dealing additional DMG.\nPlunging Attack\nGathering the might of Geo, Ningguang plunges towards the ground from mid-air, damaging all opponents in her path. Deals\nAoE Geo DMG\nupon impact with the ground.',
            data: {
                'Normal Attack DMG': {
                    Lv1: '28%',
                    Lv2: '30.1%',
                    Lv3: '32.2%',
                    Lv4: '35%',
                    Lv5: '37.1%',
                    Lv6: '39.2%',
                    Lv7: '42%',
                    Lv8: '44.8%',
                    Lv9: '47.6%',
                    Lv10: '50.4%',
                    Lv11: '53.31%',
                    Lv12: '57.12%',
                    Lv13: '60.93%',
                    Lv14: '64.74%',
                    Lv15: '68.54%',
                },
                'Charged Attack DMG': {
                    Lv1: '174.08%',
                    Lv2: '187.14%',
                    Lv3: '200.19%',
                    Lv4: '217.6%',
                    Lv5: '230.66%',
                    Lv6: '243.71%',
                    Lv7: '261.12%',
                    Lv8: '278.53%',
                    Lv9: '295.94%',
                    Lv10: '313.34%',
                    Lv11: '331.45%',
                    Lv12: '355.12%',
                    Lv13: '378.8%',
                    Lv14: '402.47%',
                    Lv15: '426.15%',
                },
                'DMG per Star Jade': {
                    Lv1: '49.6%',
                    Lv2: '53.32%',
                    Lv3: '57.04%',
                    Lv4: '62%',
                    Lv5: '65.72%',
                    Lv6: '69.44%',
                    Lv7: '74.4%',
                    Lv8: '79.36%',
                    Lv9: '84.32%',
                    Lv10: '89.28%',
                    Lv11: '94.44%',
                    Lv12: '101.18%',
                    Lv13: '107.93%',
                    Lv14: '114.68%',
                    Lv15: '121.42%',
                },
                'Charged Attack Stamina Cost': {
                    Lv1: '50',
                    Lv2: '50',
                    Lv3: '50',
                    Lv4: '50',
                    Lv5: '50',
                    Lv6: '50',
                    Lv7: '50',
                    Lv8: '50',
                    Lv9: '50',
                    Lv10: '50',
                    Lv11: '50',
                    Lv12: '50',
                    Lv13: '50',
                    Lv14: '50',
                    Lv15: '50',
                },
                'Plunge DMG': {
                    Lv1: '56.83%',
                    Lv2: '61.45%',
                    Lv3: '66.08%',
                    Lv4: '72.69%',
                    Lv5: '77.31%',
                    Lv6: '82.6%',
                    Lv7: '89.87%',
                    Lv8: '97.14%',
                    Lv9: '104.41%',
                    Lv10: '112.34%',
                    Lv11: '120.27%',
                    Lv12: '128.2%',
                    Lv13: '136.12%',
                    Lv14: '144.05%',
                    Lv15: '151.98%',
                },
                'Low Plunge DMG': {
                    Lv1: '113.63%',
                    Lv2: '122.88%',
                    Lv3: '132.13%',
                    Lv4: '145.35%',
                    Lv5: '154.59%',
                    Lv6: '165.17%',
                    Lv7: '179.7%',
                    Lv8: '194.23%',
                    Lv9: '208.77%',
                    Lv10: '224.62%',
                    Lv11: '240.48%',
                    Lv12: '256.34%',
                    Lv13: '272.19%',
                    Lv14: '288.05%',
                    Lv15: '303.9%',
                },
                'High Plunge DMG': {
                    Lv1: '141.93%',
                    Lv2: '153.49%',
                    Lv3: '165.04%',
                    Lv4: '181.54%',
                    Lv5: '193.1%',
                    Lv6: '206.3%',
                    Lv7: '224.45%',
                    Lv8: '242.61%',
                    Lv9: '260.76%',
                    Lv10: '280.57%',
                    Lv11: '300.37%',
                    Lv12: '320.18%',
                    Lv13: '339.98%',
                    Lv14: '359.79%',
                    Lv15: '379.59%',
                },
            },
        },
        {
            name: 'Jade Screen',
            image: '/images/characters/Skill_S_Ningguang_01.png',
            description:
                "Ningguang creates a Jade Screen out of gold, obsidian, and her great opulence, dealing\nAoE Geo DMG\n.\nJade Screen\n\u00b7Blocks opponents' projectiles.\n\u00b7Endurance scales based on Ningguang's Max HP.\nJade Screen is considered a\nGeo Construct\nand can be used to block certain attacks, but cannot be climbed. Only one Jade Screen may exist at any one time.\nThe canvas of stars is written upon this jade screen.",
            data: {
                'Inherited HP': {
                    Lv1: '50.1%',
                    Lv2: '53.1%',
                    Lv3: '56.1%',
                    Lv4: '60%',
                    Lv5: '63%',
                    Lv6: '66%',
                    Lv7: '69.9%',
                    Lv8: '73.8%',
                    Lv9: '77.7%',
                    Lv10: '81.6%',
                    Lv11: '85.5%',
                    Lv12: '89.4%',
                    Lv13: '93.3%',
                    Lv14: '97.2%',
                    Lv15: '101.1%',
                },
                'Skill DMG': {
                    Lv1: '230.4%',
                    Lv2: '247.68%',
                    Lv3: '264.96%',
                    Lv4: '288%',
                    Lv5: '305.28%',
                    Lv6: '322.56%',
                    Lv7: '345.6%',
                    Lv8: '368.64%',
                    Lv9: '391.68%',
                    Lv10: '414.72%',
                    Lv11: '437.76%',
                    Lv12: '460.8%',
                    Lv13: '489.6%',
                    Lv14: '518.4%',
                    Lv15: '547.2%',
                },
                CD: {
                    Lv1: '12s',
                    Lv2: '12s',
                    Lv3: '12s',
                    Lv4: '12s',
                    Lv5: '12s',
                    Lv6: '12s',
                    Lv7: '12s',
                    Lv8: '12s',
                    Lv9: '12s',
                    Lv10: '12s',
                    Lv11: '12s',
                    Lv12: '12s',
                    Lv13: '12s',
                    Lv14: '12s',
                    Lv15: '12s',
                },
            },
        },
        {
            name: 'Starshatter',
            image: '/images/characters/Skill_E_Ningguang_01.png',
            description:
                'Gathering a great number of gems, Ningguang scatters them all at once, sending homing projectiles at her opponents that deal massive\nGeo DMG\n.\nIf Starshatter is cast when a Jade Screen is nearby, the Jade Screen will fire additional gem projectiles at the same time.\nStars shatter. Silence falls.',
            data: {
                'DMG Per Gem': {
                    Lv1: '86.96%',
                    Lv2: '93.48%',
                    Lv3: '100%',
                    Lv4: '108.7%',
                    Lv5: '115.22%',
                    Lv6: '121.74%',
                    Lv7: '130.44%',
                    Lv8: '139.14%',
                    Lv9: '147.83%',
                    Lv10: '156.53%',
                    Lv11: '165.22%',
                    Lv12: '173.92%',
                    Lv13: '184.79%',
                    Lv14: '195.66%',
                    Lv15: '206.53%',
                },
                CD: {
                    Lv1: '12s',
                    Lv2: '12s',
                    Lv3: '12s',
                    Lv4: '12s',
                    Lv5: '12s',
                    Lv6: '12s',
                    Lv7: '12s',
                    Lv8: '12s',
                    Lv9: '12s',
                    Lv10: '12s',
                    Lv11: '12s',
                    Lv12: '12s',
                    Lv13: '12s',
                    Lv14: '12s',
                    Lv15: '12s',
                },
                'Energy Cost': {
                    Lv1: '40',
                    Lv2: '40',
                    Lv3: '40',
                    Lv4: '40',
                    Lv5: '40',
                    Lv6: '40',
                    Lv7: '40',
                    Lv8: '40',
                    Lv9: '40',
                    Lv10: '40',
                    Lv11: '40',
                    Lv12: '40',
                    Lv13: '40',
                    Lv14: '40',
                    Lv15: '40',
                },
            },
        },
    ],
    passiveSkills: [
        {
            name: 'Trove of Marvelous Treasures',
            image: '/images/characters/UI_Talent_Collect_Ore.png',
            description:
                'Displays the location of nearby\nore veins used in forging\non the mini-map.',
        },
        {
            name: 'Backup Plan',
            image: '/images/characters/UI_Talent_S_Ningguang_02.png',
            description:
                'When Ningguang is in possession of Star Jades, her Charged Attack does not consume Stamina.',
        },
        {
            name: 'Strategic Reserve',
            image: '/images/characters/UI_Talent_S_Ningguang_06.png',
            description:
                'A character that passes through the\nJade Screen\nwill gain a 12%\nGeo DMG Bonus\nfor 10s.',
        },
    ],
    constellations: [
        {
            name: 'Piercing Fragments',
            image: '/images/characters/UI_Talent_S_Ningguang_01.png',
            description: 'When a Normal Attack hits, it deals AoE DMG.',
            level: 1,
        },
        {
            name: 'Shock Effect',
            image: '/images/characters/UI_Talent_S_Ningguang_05.png',
            description:
                'When\nJade Screen\nis shattered, its CD will reset.\nCan occur once every 6s.',
            level: 2,
        },
        {
            name: 'Majesty Be the Array of Stars',
            image: '/images/characters/UI_Talent_U_Ningguang_02.png',
            description:
                'Increases the Level of\nStarshatter\nby 3.\nMaximum upgrade level is 15.',
            level: 3,
        },
        {
            name: 'Exquisite be the Jade, Outshining All Beneath',
            image: '/images/characters/UI_Talent_S_Ningguang_03.png',
            description:
                "Jade Screen\nincreases nearby characters' Elemental RES by 10%.",
            level: 4,
        },
        {
            name: 'Invincible Be the Jade Screen',
            image: '/images/characters/UI_Talent_U_Ningguang_01.png',
            description:
                'Increases the Level of\nJade Screen\nby 3.\nMaximum upgrade level is 15.',
            level: 5,
        },
        {
            name: 'Grandeur Be the Seven Stars',
            image: '/images/characters/UI_Talent_S_Ningguang_04.png',
            description: 'When\nStarshatter\nis used, Ningguang gains 7 Star Jades.',
            level: 6,
        },
    ],
    talentScalings,
    characterBonuses,
    constellationBonuses,
}

export default Ningguang
